<%@ Page Language="C#" AutoEventWireup="true" CodeFile="CMresultLiq.aspx.cs" Inherits="module_PSManage_Default" %>
...
<head runat="server">
    ...
    <title>公司监测结果（水）</title>
    <script type="text/javascript">
        var cmLiqArray;
        function CMresultLiq() { }
       
        CMresultLiq.BuildPanel = function (nowyear) {
            function BuildTable(cmLiqArray) {
                function Buildth(thobj) {
                    ...
                    $(CMLiqTable).find("thead").append(_newtr);
                }
                ...
                var CMLiqTable = $("#CMLiqTable");
                thArray = JSONfunc.LoadJSON("jsondata/CMLiq.txt");
                Buildth(thArray);
                for (var i in cmLiqArray)//建立表格
                {
                    var Newtr = $("<tr></tr>");
                    Buildtd(cmLiqArray[i], thArray);
                    $(CMLiqTable).find("tbody").append(Newtr);
                }
                $('#CMLiqTable').dataTable(//表格初始化
                {
                    "dom": '<fl<t>ip>',
                    ...
                    "columnDefs": [{
                        ...
                        "orderable": false,
                        "data": null,
                        "defaultContent": '<a href="#"class="btn editorbtn btn-primary" author="jidongke" data-toggle="modal" data-target="#AddModal">修改</a>'
                    }]
                });
                $('#CMLiqTable tbody').on('click', 'a.editorbtn', function () {     
                    var tr = $(this).closest('tr');
                    var table = $('#CMLiqTable').dataTable().api();
            ...
            //从数据库读取数据
            var formRead = { "action": "CMRead", "time": nowyear, "type": "Liq" };
            ...
            cmLiqArray = new Array(_retarray.length);
            for (i = 0; i < cmLiqArray.length; i++) {
                cmLiqArray[i] = new Object();
                cmLiqArray[i].Date = _retarray[i].日期;
                cmLiqArray[i].Type = _retarray[i].类型;
                cmLiqArray[i].Name = _retarray[i].监测点;
                cmLiqArray[i].Content = JSONfunc.STRtoJSON(_retarray[i].数据);
            }
            //-----------------读取结束，存入cmLiqArray---------------
            BuildTable(cmLiqArray);
            CookieFunc.Checkuser();
        }//初始化表格函数，制作新表函数
        CMresultLiq.SwapCMLiq = function (date) {
            if (date == "") {
                date = (new Date()).pattern("yyyy年");
            }
            if ($('#CMLiqTable td').length != 0)//判断是否有datatable,如果没有，则做这个函数会出错
            {
                $('#CMLiqTable').dataTable().fnDestroy();
            }
            $('#CMLiqTable').html("<thead></thead><tbody></tbody><tfoot></tfoot>");
            CMresultLiq.BuildPanel(date);
        }

        $(function () {
            var NowYear = (new Date()).pattern("yyyy年");
            CMresultLiq.BuildPanel(NowYear);
        ...
        <div class="panel panel-default">
                    ...
                    <div class="controls">
                        <div class="input-append">
                            <div class="input-group date" id="dtpclass1" data-date="" data-link-field="year-select" data-link-format="yyyy年">
                                ...
                                <a href="#" type="button" class="input-group-addon btn btn-default" onclick="CMresultLiq.SwapCMLiq($('#year-select').val())">查询</a>
                            </div>
            ...
            <ul class="list-group">
                <li class=" list-group-item">
                    <table id="CMLiqTable" class="table  table-hover  table-responsive text-center" cellspacing="0" width="100%">
                        <thead>
                        </thead>
                        <tfoot>
                        </tfoot>
                            ...
                            <div class="modal-dialog modal-lg" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-default" data-dismiss="modal"  onclick="CMresultLiq.SwapCMLiq($('#year-select').val())">关闭窗口</button>
                                    </div>
                                </div>
                            </div>
                        </div>                       
